...
{   
    ...
    m_playWidget = new PlayWidget(m_controller, this);  //创建播放界面，接收m_controller的控制信号，并显示播放信息
    ...
    m_aiController = new AiController(m_controller, this);  //创建AI控制器，控制器父对象是主窗口，窗口销毁时子对象一并释放
    ...
    connect(m_aiController->dispatcher(), &CommandDispatcher::dispatchResult, m_voiceWidget, &VoiceInputWidget::onDispatchResult);
    connect(m_aiController, &AiController::recognizing, m_voiceWidget, &VoiceInputWidget::onRecognizing);
    connect(m_aiController, &AiController::recognizeFailed, m_voiceWidget, &VoiceInputWidget::onRecognizeFailed);
}
